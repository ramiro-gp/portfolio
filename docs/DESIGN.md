...
Actualización visual H7 aprobada por Ramiro (2026-09-25), con corrección espacial implementada en H8: sólo en Light + accent negro, la línea conserva el negro sobre superficies claras y se vuelve blanca `#F1F0EC` únicamente donde su trazo y sus nodos pisan físicamente el fondo oscuro de Ramiro; al salir de esa superficie recupera el negro, incluso si ambos colores aparecen en una misma pantalla. GitHub y su outline de foco usan blanco dentro de Ramiro en esa combinación. Dark y todos los demás accents conservan exactamente el color seleccionado, incluso donde el contraste de GitHub queda por debajo de AA; esos ratios se registran como excepción de QA P2 y siguen visibles en H7/H8. Geometría, grosor, nodos, pulse, milestones y timings no cambian; reduced motion elimina sólo el pulso, no la regla espacial.
