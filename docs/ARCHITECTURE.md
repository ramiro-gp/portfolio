...
## Corrección V2-H8 — clipping espacial Light + negro (2026-09-26)

La intención aprobada en H7 es espacial: el color negro base de la línea se conserva sobre superficies claras y la pintura blanca inversa se limita al rectángulo real de la sección `.about` (Ramiro) sólo en Light + `light-black`. Un único path base es la fuente de la geometría; su representación inversa comparte exactamente `d`, dasharray, dashoffset, stroke-width, nodos, radios, progreso y pulsos (3,5 s; el pulso final conserva 0,8 s de retraso y el fade de grupo 180 ms). Un `clipPath` SVG en `userSpaceOnUse` recorta esa representación a los límites DOM actuales de `.about`; los límites se reconstruyen junto con el path en resize y cambios de layout. Esto permite que en una misma pantalla la línea sea negra antes y después de Ramiro y blanca sólo sobre su fondo oscuro. La excepción no aplica a otros accents ni a ningún accent Dark. `data-ramiro-scene` sigue atendiendo el color de GitHub y no controla globalmente la línea. QA focalizado: `qa/h8-line-clipping-checks.cjs`, resultados/capturas en `qa/h8-line-clipping-results.json` y `qa/h8-line-clipping/`.
