...
La fuente local Instrument Sans carece de glifos japoneses según su `cmap` (muestras 日、本、語、あ、ポ). La solución tipográfica H4 fija Noto Sans JP como fallback sólo para caracteres japoneses, con Instrument Sans primero para latín. La fuente de revisión local genérica y su licencia OFL están en `prototypes/v2-h4/v2/assets/`; no es un asset optimizado de producción. En H6 se autoalojará una versión WOFF2 con cobertura de todo el corpus JA aprobado, `font-display: swap`, pesos 400/600, sin solicitar al proveedor texto privado para generar subsets. Verificar carga, cobertura, ruptura de línea y rendering tras integrar el copy final; la ratificación técnica de carga pertenece a H5/H6.
